...
class ContractWarranty3 extends Controller
{
    ...
    protected function grid()
    {
        ...
        $grid->id('Id');
        $grid->model()->where(DB::raw('DATE_ADD(selected_at, INTERVAL 9 MONTH)'), '>', Carbon::now())->orderBy('selected_at', 'asc');
        ...
        $grid->selected_at('Trạng Thái Bảo Trì')->display(function ($date) {
            $countdown = Carbon::now()->diffInDays(Carbon::parse($date)->addMonth(9), false);
